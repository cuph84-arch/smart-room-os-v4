...
function startRealtimeListener() {
  onValue(
    ...
    error => {
      ...
      showToast("Gagal memuat data realtime");
    }
  );
}
...
function renderTV(tv) {
  ...
  const displayStatus = tv.displayStatus || "Screen Saver";
...
function bindControls() {
  ...
  if (tvPowerToggle) {
    tvPowerToggle.addEventListener("change", () => {
      const nextPower = tvPowerToggle.checked ? "ON" : "OFF";

      update(ref(database, "state/tv"), {
        power: nextPower,
        updated_at: new Date().toISOString()
      }).catch(error => {
        console.error("TV power update error:", error);
        showToast("Gagal update TV");
      });
    });
  }
  ...
  if (tvVolumeSlider) {
    ...
    tvVolumeSlider.addEventListener("change", () => {
      ...
      update(ref(database, "state/tv"), {
        ...
        console.error("TV volume update error:", error);
        showToast("Gagal update volume TV");
      });
    });
  }
